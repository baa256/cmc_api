...
class Crypto(dict):
    ...
    def __init__(self, symbol, api: CoinMarketCapAPI):
        quote_data = api.get_cryptocurrencies([symbol])[0]
        metadata = api.get_metadata([symbol])[0]
        fcas_score_data = api.get_fcas_score(symbol)
...
if __name__ == '__main__':
    ...
    # prime the api
    top_cryptos = api.get_top_cryptos(500)
    symbols = [x['symbol'] for x in top_cryptos]
    api.get_cryptocurrencies(symbols)
    api.get_metadata(symbols)
    api.get_fcas_scores(pages=10, limit=100)
